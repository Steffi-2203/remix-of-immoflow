...
export async function bulkUpsertLines(opts: BulkUpsertOpts): Promise<BulkUpsertResult> {
  ...
  // Step 2: Insert into temp table in 5000-row chunks
  ...
  // Step 3: Single CTE – upsert + audit in one roundtrip
  ...
  const upsertedLinesCount = Number(row.upserted_count || 0);
  const conflictCount = allLines.length - upsertedLinesCount;

  return { upsertedLinesCount, conflictCount };
}
