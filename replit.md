...
- **Financial Audit Logging**: GoBD-compliant SHA-256 hash chain via `auditHashService.ts`. Logged actions: `payment_created`, `payment_updated` (with previousAmount/newAmount/changedFields), `payment_deleted`, `allocation_created`, `allocation_deleted`. All entries stored in `financial_audit_log` table with organization/user context.
...
- **CI Workflows**: `.github/workflows/ci.yml` (unit tests + Postgres service), `.github/workflows/e2e-browser.yml` (Playwright E2E with seed/teardown, nightly schedule, PR trigger, fails on missing `TEST_STRIPE_WEBHOOK_SECRET`), `.github/workflows/dryrun-persist-ci.yml`.
- **Nightly Reconciliation**: `.github/workflows/nightly-reconcile.yml` — vergleicht `paid_amount` mit `payment_allocations`, alertiert via Slack/PagerDuty bei Abweichungen >0.01 EUR. Warn bei >0 Varianzen, Fail bei >1000 (temporarily raised from >55 to reduce noise from known seed data — TODO: revert once seed rows excluded from SQL). Reports seed_count as diagnostic metric. Secrets guard: fails immediately with clear error if `STAGING_DB_URL` not set. Report: `scripts/reconcile_paid_amounts.sql` (includes source_breakdown section). Runbook: `ops/runbooks/reconciliation.md`.
- **Seed Source Tagging**: `payments` and `payment_allocations` have `source` column (default 'manual'). Seed scripts tag with `source='seed'` and include verification query at end (warns if orphan allocations found). Schema migration: `migrations/2026-02-24-add-source-to-payments.sql`. Backfill: `migrations/2026-02-24-backfill-seed-source.sql` (tags 649 payments, creates 441 missing allocations). Safety-net backfill: `migrations/2026-02-24-backfill-create-allocations-from-paid-amounts.sql`. Fixture: `fixtures/seed-payments.sql`.
...
- **Supabase Cleanup (2026-02-24)**: All legacy Supabase Edge Functions (11 functions), 44 migrations, config.toml, and dead client integration (`src/integrations/supabase/`) removed. All functionality replaced by Express endpoints. `@supabase/supabase-js` npm package remains but is unused — can be removed in future cleanup.
