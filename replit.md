...
- **Unit Testing Infrastructure**: Vitest-based test suite (249 tests across 21 files) covering WEG USt calculation (10%/20%/0%), MEA distribution, budget line categorization, SEPA XML validation, invoice generation, payment allocation (FIFO), dunning (ABGB §1333), deposit lifecycle, pro-rata billing, bank reconciliation, ledger sync, payment storno, concurrent edge cases, security guards, tenant isolation, GoBD audit trails, E2E billing cycles, and health endpoints. CI pipeline (`.github/workflows/ci.yml`) runs on push/PR with Postgres service. Config: `vitest.config.ts`, tests in `tests/unit/`.
...
- **DB Performance**: 16 performance indexes on frequently queried columns. Dedicated `extensions` schema for PostgreSQL extensions (migration `20260213_create_extensions_schema.sql`); new extensions should be created with `CREATE EXTENSION ... SCHEMA extensions`.
