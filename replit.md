...
- **Nightly Reconciliation**: `.github/workflows/nightly-reconcile.yml` — vergleicht `paid_amount` mit `payment_allocations`, alertiert via Slack/PagerDuty bei Abweichungen >0.01 EUR. Warn bei >0 Varianzen, Fail bei >55. Reports seed_count as diagnostic metric. Report: `scripts/reconcile_paid_amounts.sql` (includes source_breakdown section). Runbook: `ops/runbooks/reconciliation.md`.
