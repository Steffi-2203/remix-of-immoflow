...
- **Nightly Reconciliation**: `.github/workflows/nightly-reconcile.yml` — vergleicht `paid_amount` mit `payment_allocations` (excl. `source='seed'`), alertiert via Slack/PagerDuty bei Abweichungen >0.01 EUR. Warn bei >0 Varianzen, Fail bei >55. Report: `scripts/reconcile_paid_amounts.sql`. Runbook: `ops/runbooks/reconciliation.md`. Migration: `migrations/2026-02-24-add-source-to-payments.sql`.
