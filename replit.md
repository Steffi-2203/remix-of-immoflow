...
- **Nightly Reconciliation**: `.github/workflows/nightly-reconcile.yml` — vergleicht `paid_amount` mit `payment_allocations`, alertiert via Slack/PagerDuty bei Abweichungen >0.01 EUR. Threshold: >55 variances (50 known seed-data + 5 buffer). Report: `scripts/reconcile_paid_amounts.sql`. Runbook: `ops/runbooks/reconciliation.md`.
