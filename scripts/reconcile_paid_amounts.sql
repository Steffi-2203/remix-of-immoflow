SELECT
  ...
  SUM(gesamt_betrag::numeric) AS total_invoiced,
  SUM(paid_amount) AS total_paid,
  SUM(gesamt_betrag::numeric) - SUM(paid_amount) AS total_outstanding
...
LEFT JOIN (
  SELECT invoice_id, SUM(amount::numeric) AS alloc_total
...
SELECT
  ...
  mi.gesamt_betrag,
  CASE
    WHEN mi.paid_amount >= mi.gesamt_betrag::numeric AND mi.status != 'bezahlt' THEN 'MISMATCH: should be bezahlt'
    WHEN mi.paid_amount > 0 AND mi.paid_amount < mi.gesamt_betrag::numeric AND mi.status NOT IN ('teilbezahlt', 'bezahlt') THEN 'MISMATCH: should be teilbezahlt'
...
WHERE status != 'storniert'
  AND (
    (mi.paid_amount >= mi.gesamt_betrag::numeric AND mi.status != 'bezahlt')
    OR (mi.paid_amount > 0 AND mi.paid_amount < mi.gesamt_betrag::numeric AND mi.status NOT IN ('teilbezahlt', 'bezahlt'))
...
SELECT
  ...
  mahnstufe,
  COUNT(*) AS count,
  SUM(gesamt_betrag::numeric - paid_amount) AS outstanding_amount
FROM monthly_invoices
