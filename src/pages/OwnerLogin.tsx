...
export default function OwnerLogin() {
  ...
  async function checkSession() {
    try {
      const res = await fetch('/api/owner-auth/session', { credentials: 'include' });
      const data = await res.json();
      if (data.authenticated) {
        navigate('/eigentuemer-portal', { replace: true });
      }
    } catch {}
  }
  ...
  async function checkInvite(token: string) {
    try {
      const res = await fetch(`/api/owner-auth/invite/${token}`, { credentials: 'include' });
      const data = await res.json();
      if (data.valid) {
        setInviteValid(true);
  ...
  async function handleSetPassword(e: React.FormEvent) {
    ...
    setIsSubmitting(true);
    try {
      const res = await fetch('/api/owner-auth/set-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        ...
        <Card className="w-full max-w-md">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground text-center">
              Ihr Eigentümerportal-Zugang wurde erfolgreich eingerichtet. Sie werden weitergeleitet...
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center space-y-4">
            ...
            {mode === 'login' ? (
              <>
                <CardTitle className="text-2xl font-bold" data-testid="text-owner-login-title">
                  Eigentümerportal
                </CardTitle>
                <CardDescription>
              ...
              <>
                <CardTitle className="text-2xl font-bold" data-testid="text-owner-set-password-title">
                  Zugang einrichten
                </CardTitle>
                <CardDescription>
                  Setzen Sie Ihr Passwort für das Eigentümerportal
                </CardDescription>
                {inviteEmail && (
            ...
            {mode === 'login' ? (
              <form onSubmit={handleLogin} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    id="email"
                    type="email"
                    placeholder="eigentuemer@beispiel.at"
                    value={email}
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Button
                      ...
                      data-testid="button-owner-toggle-password"
                    >
              ...
              <form onSubmit={handleSetPassword} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Button
                      ...
                      className="absolute right-0 top-0"
                      onClick={() => setShowPassword(!showPassword)}
                      data-testid="button-owner-toggle-new-password"
                    >
                      {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                    </Button>
                  </div>
                ...
                <Button
                  ...
                  className="w-full"
                  disabled={isSubmitting}
                  data-testid="button-owner-set-password"
                >
                  {isSubmitting ? (
                    <Loader2 className="h-4 w-4 animate-spin mr-2" />
                  ) : (
          ...
          <CardFooter className="flex flex-col gap-3 text-center">
            ...
            <Link to="/" className="text-sm text-primary hover:underline" data-testid="link-owner-back-home">
