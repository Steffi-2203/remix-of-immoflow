...
const PRODUCT_TO_TIER: Record<string, string> = {
  ...
  "prod_TjJFbvPbDQsy3j": "enterprise", // Premium maps to enterprise tier in DB
};
...
serve(async (req) => {
  ...
  try {
    ...
    if (customers.data.length === 0) {
      logStep("No Stripe customer found, returning trial status");
    ...
    if (hasActiveSub) {
      ...
      if (profile?.organization_id && subscriptionTier) {
